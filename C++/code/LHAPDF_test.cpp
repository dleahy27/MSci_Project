...
int main(int argc, char* argv[]){
    ...
    spline_ds_num.outputDerivs("ct10_derivs.csv");
    // write the data to file
    std::ofstream myfile;
    myfile.open("../outputs/CT10nlo.csv");
    myfile << "x,y,z\n";
    for (  int i = 0; i<y_n; i++){
        for (  int j = 0; j<x_n; j++){
            myfile<<xs_t[j]<<","<<ys_t[i]<<","<<zs_spline_ds_num[i][j]<< std::endl;
        }
    }
    myfile.close();
