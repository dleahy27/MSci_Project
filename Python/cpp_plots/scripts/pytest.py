...
if __name__ == '__main__':
    # read in data
    x_c,y_c,z_c,z_cds,z_cds_num,z_cspline = np.loadtxt("/mnt/c/Users/dillo/Desktop/work/Uni/Year_5/Project/Code/MSci_Project/C++/outputs/trig_bicubic.csv", dtype='float', delimiter=',', unpack=True, skiprows=1)
    ...
    z_cds_num = z_cds.reshape((sizey,sizex))
    ...
    fig.tight_layout()
    fig.savefig("../outputs/trig/function_plots.pdf",dpi=800, format="pdf")
    ...
    plot2 = axs[1].contourf(x_c,y_c,ratio, levels=levels, norm=norm, cmap="seismic")
    axs[1].set_title(r"Error with second derivatives = 0", fontsize=title_size)
    axs[1].label_outer()
    # axs[1].set_xscale("log")
    # axs[1].set_yscale("log")
    
    plot3 = axs[2].contourf(x_c,y_c,ratio_ds, levels=levels, norm=norm, cmap="seismic")
    axs[2].set_title(r"Error with analytic second derivatives", fontsize=title_size)
    axs[2].label_outer()
    # axs[2].set_xscale("log")
    # axs[2].set_yscale("log")
    
    plot4 = axs[3].contourf(x_c,y_c,ratio_ds_num, levels=levels, norm=norm, cmap="seismic")
    ...
    cb3 = plt.colorbar(plot3, ax=axs[2], ticks=ticks1)
    cb4 = plt.colorbar(plot4, ax=axs[3], ticks=ticks1)
    ...
    fig.supylabel(r'y', x=0.01, fontsize=axes_size) 
    fig.tight_layout()
    fig.savefig("../outputs/trig/func_and_err.pdf", dpi=800, format="pdf")
    ...
    # plot1 = axs[0].contourf(x_c,y_c,ratio, levels=levels, norm=norm, cmap="seismic")
    # axs[0].set_title(r"second derivatives = 0", fontsize=title_size)
    # axs[0].label_outer()
    
    # plot2 = axs[1].contourf(x_c,y_c,ratio_ds, levels=levels, norm=norm, cmap="seismic")
    # axs[1].set_title(r"analytic second derivatives", fontsize=title_size)
    # axs[1].label_outer()
    
    # plot3 = axs[2].contourf(x_c,y_c,ratio_ds_num, levels=levels, norm=norm, cmap="seismic")
    ...
    # plot1 = axs[0].contourf(x_c,y_c,ratio_ds, levels=levels, norm=norm, cmap="seismic")
    # axs[0].set_title(r"analytic second derivatives", fontsize=title_size)
    
    # plot2 = axs[1].contourf(x_c,y_c,ratio_ds_num, levels=levels, norm=norm, cmap="seismic")
